...
int gk_Factory_RTC_Boot = 0;

extern void rtc_mart_factory_mode_alarm(void);
static ssize_t factory_mode_show(char* buf){
	printk("factory_mode_show: factory_mode_show %d\n",gk_Factory_RTC_Boot);
	return sprintf(buf,"%d\n",gk_Factory_RTC_Boot);
}
static ssize_t factory_mode_store(const char* buf,size_t len){

	printk("factory_mode_store will call rtc_mart_factory_mode_alarm\n");
	rtc_mart_factory_mode_alarm();
	
	return len;
}

#define FACTORY_RTC_ATTR         "factory_rtc"
...
struct attribute factory_rtc_attr = {FACTORY_RTC_ATTR, 0644};
struct attribute info_attr = {INFO_SYSFS_ATTR, 0644};
static struct attribute *boot_attrs[] = {
    &boot_attr,
    &md_attr,
    &factory_rtc_attr, //kaka_11_0622
    &info_attr,
    NULL
};
