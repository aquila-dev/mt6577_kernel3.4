...
bool hwPowerOn(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt, char *mode_name)
{
	...
	{		
		for(j=0 ; j<MT65XX_POWER_COUNT_END ; j++)
		{
			...
			{
				sprintf(g_MT6577_BusHW.Power[j].mod_name[k] , "%s", NON_OP);
			}
			g_MT6577_BusHW.Power[j].dwPowerCount=0;
		}
    ...
    {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[hwPowerOn] %d,%s,%d\r\n", i, g_MT6577_BusHW.Power[powerId].mod_name[i], g_MT6577_BusHW.Power[powerId].dwPowerCount);
	
        if (!strcmp(g_MT6577_BusHW.Power[powerId].mod_name[i], NON_OP))
        {
            MSG(PMIC,"[%s] acquire powerId:%d index:%d mod_name: %s\r\n", 
                __FUNCTION__,powerId, i, mode_name);            
            sprintf(g_MT6577_BusHW.Power[powerId].mod_name[i] , "%s", mode_name);
            break ;
        }
        /* already it */
        #if 0
        else if (!strcmp(g_MT6577_BusHW.Power[powerId].mod_name[i], mode_name))
        {
            MSG(CG,"[%d] Power already register\r\n",powerId );        
        }
        #endif
    }    
    g_MT6577_BusHW.Power[powerId].dwPowerCount++ ;
    /* We've already enable this LDO before */
    if(g_MT6577_BusHW.Power[powerId].dwPowerCount > 1)
    {
    	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[hwPowerOn] g_MT6577_BusHW.Power[powerId].dwPowerCount (%d) > 1\r\n", g_MT6577_BusHW.Power[powerId].dwPowerCount);
        return TRUE;
    }
#endif	
    /* Turn on PMU LDO*/
...
bool hwPowerDown(MT65XX_POWER powerId, char *mode_name)
{
    ...
    if(g_MT6577_BusHW.Power[powerId].dwPowerCount == 0)
    {
        MSG(PMIC,"%s:%s:%d powerId:%d (g_MT6577_BusHW.dwPowerCount[powerId] = 0)\r\n", 
            __FILE__,__FUNCTION__,__LINE__ ,powerId);
        // luming_20120328 changed from FALSE to TRUE
        // For Power is down,and this is desired.
        return TRUE;
    }
    for (i = 0; i< MAX_DEVICE; i++)
    {
        ...
        if (!strcmp(g_MT6577_BusHW.Power[powerId].mod_name[i], mode_name))
        {
            MSG(PMIC,"[%s] powerId:%d index:%d mod_name: %s\r\n", 
                __FUNCTION__,powerId, i, mode_name);            
            sprintf(g_MT6577_BusHW.Power[powerId].mod_name[i] , "%s", NON_OP);
            bFind = TRUE;
            break ;
        }
    }   
    ...
    if(g_MT6577_BusHW.Power[powerId].dwPowerCount > 0)
    {
    	xlog_printk(ANDROID_LOG_DEBUG, "Power/PMIC", "[hwPowerDown] g_MT6577_BusHW.Power[powerId].dwPowerCount (%d) > 0\r\n", g_MT6577_BusHW.Power[powerId].dwPowerCount);
        return TRUE;
    }
#endif	
    /* Turn off PMU LDO*/
