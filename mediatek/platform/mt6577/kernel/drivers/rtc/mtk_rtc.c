 ...
 *     bit 6 - 14: reserved bits
 *     bit 15    : DPIDLE_SHUTDOWN bit
 */
...
//kaka_11_0622
#define HQ_FACTORY_RTC_BIT	7  //kaka_12_0330   bit 7 now is reserved 
void rtc_mart_factory_mode_alarm(void){
	u16 pdn2;

	spin_lock_irq(&rtc_lock);
	pdn2 = rtc_read(RTC_PDN2) & ~(1 << HQ_FACTORY_RTC_BIT); //kaka_12_0330
	rtc_write(RTC_PDN2, pdn2 | (1 << HQ_FACTORY_RTC_BIT)); //kaka_12_0330 //factory_mode alarm
	rtc_write_trigger();
...
void rtc_read_pwron_alarm(struct rtc_wkalrm *alm)
{
	...
	if (rtc_show_alarm) {
		//kaka_11_0819 add
		//We print RTC_PDN1, RTC_PDN2,RTC_SPAR1 regiters for some bugs.
		printk("[%s] RTC_PDN1:0x%.8x RTC_PDN2:0x%.8x RTC_SPAR1:0x%.8x\n"
			,__func__
			,pdn1
			,pdn2
			,spar1);
