...
UINT32 DSI_dcs_read_lcm_reg_v2(UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
    ...
    do
    {
	  ...
	  return 0;
       ...
       {
	#ifdef HQ_LCD_ESD_CHECK
             if(DSI_CMDQ_REG->data0[0].byte2 == 0x0a && DSI_REG->DSI_RX_DATA.BYTE1 == 0x9c)
             {	
		if(buffer_size<6)
			recv_data_cnt = buffer_size;
		else
			recv_data_cnt = 6;
		memcpy((void*)buffer, (void*)&DSI_REG->DSI_RX_DATA.BYTE1, recv_data_cnt);	
             }
             else
	#endif
             {
